...
train_dict = {
    ...
    'save_location' : 'F3/fault'    # file name for the saved trained model
}

# Define the dictionary holding all the prediction parameters
pred_dict = {
    'keras_model'   : None, #keras.models.load_model('F3/fault.h5'), # input model to be used for prediction, to load a model use: keras.models.load_model('write_location')
    'section_edge'  : np.asarray([150, 700, 750, 750, 150, 1700]), # inline and xline section to be predicted (all depths), must contain xline
    ...
    'save_location' : 'predictions/fault_prediction',       # file name for the saved prediction
