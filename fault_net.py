...
from fault_net_func.masterf import *
...
train_dict = {
    'files'         : ['./class_addresses/multi_else_ilxl.pts','./class_addresses/f3_fault_training_locations.pts'], # list of names of class-adresses
    'epochs'        : 25,       # number of epochs we run on each training ensemble/mini-batch
    'num_train_ex'  : 22000,    # number of training examples in each training ensemble/mini-batch
    'batch_size'    : 50,       # number of training examples fed to the optimizer as a batch
    'val_split'     : 0.3,      # fraction of examples used for validation
    'opt_patience'  : 25,       # number of epochs with the same accuracy before force breaking the training ensemble/mini-batch
    ...
    'save_location' : 'F3/fault25epochs_tall_baseline'    # file name for the saved trained model
}

# Define the dictionary holding all the prediction parameters
pred_dict = {
    'keras_model'   : None, #keras.models.load_model('F3/fault25epochs_tall_test.h5'), # input model to be used for prediction, to load a model use: keras.models.load_model('write_location')
    'section_edge'  : np.asarray([150, 700, 350, 1200, 300, 1500]), # inline and xline section to be predicted (all depths), must contain xline
    'show_feature'  : False,     # Show the distinct features before they are combined to a prediction
    'xline'         : 750,      # xline used for classification (index)(should be within section range)
    ...
    'save_location' : 'predictions/F3_fault25epochs_tall_baseline',       # file name for the saved prediction
...
for i in range(len(liste)):
    aug = [liste1[i]]
    name = 'F3/fault25epochs_tall_' + liste[i]
...
name = 'F3/fault25epochs_tall_' + 'mirror1_2_3_T'
train_dict['data_augmentation'] = aug
train_dict['save_location'] = name
